...
| POST   | `/api/books/:id/file`       | owner | raw `application/octet-stream` bytes (`X-File-Name` header)                                   | `201 { data: BookFileMeta }` — streamed to disk, kind sniffed from magic bytes (PDF/EPUB/audio), `BOOK_FILE_MAX_BYTES` cap; same kind replaces |
| GET    | `/api/books/:id/file/:kind` | owner | — (honours `Range: bytes=…`)                                                                  | `200` full body, or `206` + `Content-Range` for a range; `416` past EOF. Audio seeks and pdf.js lazy fetches depend on this                    |
| DELETE | `/api/books/:id/file/:kind` | owner | —                                                                                             | `204` — removes the row and the bytes on disk                                                                                                  |
| GET    | `/api/books/:id/progress`   | owner | —                                                                                             | `200 { data: { position, percent, updatedAt } \| null }`                                                                                       |
| PUT    | `/api/books/:id/progress`   | owner | `{ position, percent }`                                                                       | `200 { data: ReadingProgress }` (upsert; position is a page number, CFI, or seconds by medium)                                                 |

`format` is `PHYSICAL | EBOOK | AUDIOBOOK` — a label for the reader's own
filtering, deliberately decoupled from any attached file: a book of any
format may hold one file per kind (`PDF | EPUB | AUDIO`), so a physical book
can carry its scan. Files and reading progress sit inside the privacy
boundary with the journal: shelf contributors and per-book share recipients
get 404 on every file and progress endpoint (pinned by tests), and the E18
recipient projection stays metadata-only.
